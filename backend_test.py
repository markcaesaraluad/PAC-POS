...
Test Import Product 2,IMP-002,1234567890124,Books,5.00,12.99,25,active,Second imported product,Test Brand 2,Test Supplier 2,10"""
    ...
    def investigate_deployed_login_issues(self):
        """Comprehensive investigation of deployed version login issues as requested"""
        self.log("=== STARTING DEPLOYED VERSION LOGIN INVESTIGATION ===", "INFO")
        
        # 1. Database Super Admin Verification
        self.log("🔍 1. DATABASE SUPER ADMIN VERIFICATION", "INFO")
        self.verify_super_admin_database()
        
        # 2. Authentication Endpoint Testing
        self.log("🔍 2. AUTHENTICATION ENDPOINT TESTING", "INFO")
        self.test_authentication_endpoints()
        
        # 3. Business Admin Verification
        self.log("🔍 3. BUSINESS ADMIN VERIFICATION", "INFO")
        self.verify_business_admin_setup()
        
        # 4. Environment Configuration Check
        self.log("🔍 4. ENVIRONMENT CONFIGURATION CHECK", "INFO")
        self.check_environment_configuration()
        
        # 5. Error Message Analysis
        self.log("🔍 5. ERROR MESSAGE ANALYSIS", "INFO")
        self.analyze_login_error_messages()
        
        self.log("=== DEPLOYED VERSION LOGIN INVESTIGATION COMPLETED ===", "INFO")
        self.print_summary()
        return True

    def verify_super_admin_database(self):
        """Verify super admin user exists in database with correct credentials"""
        self.log("Testing Super Admin Database State", "INFO")
        
        # Test super admin login with expected credentials
        success, response = self.run_test(
            "Super Admin Login - admin@pos.com",
            "POST",
            "/api/auth/login",
            [200, 401, 404, 422, 500],  # Accept multiple status codes for analysis
            data={
                "email": "admin@pos.com",
                "password": "admin123"
            }
        ...
        if success and response.get('access_token'):
            self.super_admin_token = response['access_token']
            self.token = self.super_admin_token
            self.log("✅ Super admin exists and login successful")
            self.log(f"Token received: {response['access_token'][:50]}...")
            ...
            if 'user' in response:
                ...
                if user.get('role') == 'super_admin':
                    self.log("✅ Super admin role confirmed")
                else:
                    self.log(f"❌ Unexpected role: {user.get('role')}")
            
            # Test /api/auth/me endpoint with super admin token
            success, me_response = self.run_test(
                "Super Admin - Get Current User",
                "GET",
                "/api/auth/me",
                200
            )
            ...
            if success:
                self.log("✅ Super admin token validation working")
                self.log(f"Current user: {me_response.get('email')} ({me_response.get('role')})")
            else:
                self.log("❌ Super admin token validation failed")
                
        else:
            self.log("❌ CRITICAL: Super admin login failed")
            self.log(f"Response: {json.dumps(response, indent=2)}")
            
            # Try alternative credentials
            alt_credentials = [
                {"email": "admin@pos.com", "password": "admin123456"},
            ...
            for creds in alt_credentials:
                success, response = self.run_test(
                    f"Super Admin Login Alternative - {creds['email']}",
                    "POST",
                    "/api/auth/login",
                    [200, 401, 404, 422, 500],
                    data=creds
                ...
                if success and response.get('access_token'):
                    self.log(f"✅ Alternative credentials work: {creds}")
                    self.super_admin_token = response['access_token']
                    break

    def test_authentication_endpoints(self):
        """Test authentication endpoints directly with both credential sets"""
        self.log("Testing Authentication Endpoints Directly", "INFO")
        
        # Test 1: Super Admin Login with exact error capture
        self.log("Testing Super Admin Login with Error Capture", "INFO")
        success, response = self.run_test(
            "Super Admin Login - Detailed Error Capture",
            "POST",
            "/api/auth/login",
            [200, 401, 404, 422, 500],  # Accept multiple status codes
            data={
                "email": "admin@pos.com",
                "password": "admin123"
            }
        ...
        self.log(f"Super Admin Login Response: {json.dumps(response, indent=2)}")
        
        # Test 2: Business Admin Login with exact error capture
        self.log("Testing Business Admin Login with Error Capture", "INFO")
        success, response = self.run_test(
            "Business Admin Login - Detailed Error Capture",
            "POST",
            "/api/auth/login",
            [200, 401, 404, 422, 500],  # Accept multiple status codes
            data={
        ...
        self.log(f"Business Admin Login Response: {json.dumps(response, indent=2)}")
        
        # Test 3: JWT Token Generation and Validation
        if self.super_admin_token or self.business_admin_token:
            test_token = self.super_admin_token or self.business_admin_token
            self.token = test_token
            
            success, response = self.run_test(
                "JWT Token Validation - /api/auth/me",
                "GET",
                "/api/auth/me",
                [200, 401, 403, 422]
            )
            ...
            if success:
                ...
                self.log(f"Token validation response: {json.dumps(response, indent=2)}")
            else:
                self.log("❌ JWT token validation failed")
                self.log(f"Token validation error: {json.dumps(response, indent=2)}")

    def verify_business_admin_setup(self):
        """Verify business admin exists and is properly linked to business"""
        self.log("Verifying Business Admin Setup", "INFO")
        
        # First, try to login as business admin
        success, response = self.run_test(
            "Business Admin Login Verification",
            "POST",
            "/api/auth/login",
        ...
        if success and response.get('access_token'):
            ...
            if 'user' in response:
                user = response['user']
                self.log(f"Business Admin User ID: {user.get('id')}")
                self.log(f"Business Admin Email: {user.get('email')}")
                self.log(f"Business Admin Role: {user.get('role')}")
                
                if 'business_id' in user:
                    business_id = user['business_id']
                    self.log(f"Business ID: {business_id}")
                    
                    # Switch to business admin token and verify business access
                    self.token = self.business_admin_token
                    
                    success, business_response = self.run_test(
                        "Get Business Info with Business Admin Token",
                        "GET",
                        "/api/business/info",
                        [200, 401, 403, 404, 500]
                    )
                    ...
                    if success:
                        self.log("✅ Business admin can access business endpoints")
                        self.log(f"Business Info: {json.dumps(business_response, indent=2)}")
                        
                        # Verify subdomain mapping
                        if business_response.get('subdomain') == 'prints-cuts-tagum':
                            self.log("✅ Subdomain mapping correct")
                        else:
                            self.log(f"❌ Subdomain mismatch: expected 'prints-cuts-tagum', got '{business_response.get('subdomain')}'")
                            
                        # Verify business is active
                        if business_response.get('is_active', True):
                            self.log("✅ Business is active")
                        else:
                            self.log("❌ Business is suspended/inactive")
                    else:
                        self.log("❌ Business admin cannot access business endpoints")
                        self.log(f"Business access error: {json.dumps(business_response, indent=2)}")
                else:
                    self.log("❌ No business_id in user response")
            else:
                self.log("❌ No user data in login response")
        else:
            self.log("❌ Business admin login failed")
            self.log(f"Error response: {json.dumps(response, indent=2)}")
            
            # Try without subdomain
            success, response = self.run_test(
                "Business Admin Login Without Subdomain",
                "POST",
                "/api/auth/login",
                [200, 401, 404, 422, 500],
                data={
            ...
            if success and response.get('access_token'):
                self.log("✅ Business admin login works without subdomain")
                self.business_admin_token = response['access_token']
            else:
                self.log("❌ Business admin login fails even without subdomain")
                self.log(f"No subdomain error: {json.dumps(response, indent=2)}")
    ...
    def check_environment_configuration(self):
        ...
        # Test 1: Health check to verify backend is running
        success, response = self.run_test(
        ...
        if success:
            ...
            self.log(f"Health response: {json.dumps(response, indent=2)}")
        else:
            self.log("❌ Backend health check failed")
            
        # Test 2: Database connectivity test (if available)
        # Try to access an endpoint that requires database
        if self.super_admin_token:
            self.token = self.super_admin_token
            success, response = self.run_test(
                "Database Connectivity Test - List Businesses",
                "GET",
                "/api/super-admin/businesses",
                [200, 401, 403, 500]
            )
            ...
            if success:
                ...
                if isinstance(response, list):
                    self.log(f"Found {len(response)} businesses in database")
                    for business in response:
                        self.log(f"  - Business: {business.get('name')} (subdomain: {business.get('subdomain')}, active: {business.get('is_active')})")
            else:
                self.log("❌ Database connectivity issues detected")
                self.log(f"Database error: {json.dumps(response, indent=2)}")
            
        # Test 3: Environment-specific URL verification
        self.log(f"Testing against URL: {self.base_url}")
        if "preview.emergentagent.com" in self.base_url:
            self.log("✅ Using correct deployed environment URL")
        else:
            self.log("⚠️ URL might not be the deployed environment")

    def analyze_login_error_messages(self):
        """Capture and analyze exact error messages from failed login attempts"""
        self.log("Analyzing Login Error Messages", "INFO")
        
        # Test various failure scenarios to capture exact error messages
        test_scenarios = [
            {
                "name": "Wrong Super Admin Password",
                "data": {"email": "admin@pos.com", "password": "wrongpassword"}
            },
            {
                "name": "Wrong Business Admin Password", 
                "data": {"email": "admin@printsandcuts.com", "password": "wrongpassword", "business_subdomain": "prints-cuts-tagum"}
            },
            {
            ...
            },
            {
                ...
                "data": {"email": "admin@printsandcuts.com", "password": "admin123456", "business_subdomain": "wrong-subdomain"}
            },
            {
                "name": "Missing Subdomain for Business User",
                "data": {"email": "admin@printsandcuts.com", "password": "admin123456"}
            },
            {
        ...
        for scenario in test_scenarios:
            self.log(f"Testing: {scenario['name']}", "INFO")
            success, response = self.run_test(
                scenario['name'],
                "POST",
                "/api/auth/login",
                [200, 400, 401, 404, 422, 500],  # Accept any status
                data=scenario['data']
            )
            
            self.log(f"Response for {scenario['name']}: {json.dumps(response, indent=2)}")
        ...
        self.log(f"Invalid token response: {json.dumps(response, indent=2)}")
