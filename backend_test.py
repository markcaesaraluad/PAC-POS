...
def main():
    ...
    def test_today_date_filtering_issue(self):
        ...
        if success:
            content_length = len(response) if isinstance(response, bytes) else 0
            self.log(f"✅ PDF sales report for today: {content_length} bytes")
            if content_length > 1000:  # Reasonable size for PDF with data
                self.log("✅ PDF report appears to contain data (size > 1KB)")
            else:
                self.log("⚠️ PDF report may be empty or contain minimal data")
        ...
        if success:
            content_length = len(response) if isinstance(response, bytes) else 0
            self.log(f"✅ CSV profit report for today: {content_length} bytes")
            if content_length > 500:
                self.log("✅ CSV profit report appears to contain data")
            else:
                self.log("⚠️ CSV profit report may be empty")
        ...
        success, response = self.run_test(
            "Sales Report - Explicit End Time",
            "GET",
            "/api/reports/sales",
            200,
            params={
                "format": "excel",
                "start_date": today_date_str + "T00:00:00",
                "end_date": today_date_str + "T23:59:59"
            }
        )
        
        if success:
            content_length = len(response) if isinstance(response, bytes) else 0
            self.log(f"✅ Sales report with explicit end time: {content_length} bytes")
        ...
        if success and isinstance(response, dict):
            yesterday_sales = response.get('sales', {}).get('total_sales', 0)
            yesterday_revenue = response.get('sales', {}).get('total_revenue', 0)
            
            self.log(f"✅ Daily Summary for yesterday:")
            self.log(f"   - Total Sales: {yesterday_sales}")
            self.log(f"   - Total Revenue: ${yesterday_revenue}")
        
        # TEST 6: Test Different Date Formats
        self.log("🔍 TEST 6: Test Different Date Formats", "INFO")
        
        date_formats = [
            today_date_str,  # YYYY-MM-DD
            today_iso_str,   # YYYY-MM-DDTHH:MM:SS
            today_start.isoformat(),  # Start of day
            today_end.isoformat()     # End of day
        ]
        
        for i, date_format in enumerate(date_formats):
            success, response = self.run_test(
                f"Sales Report - Date Format {i+1}: {date_format[:19]}",
                "GET",
                "/api/reports/sales",
                200,
                params={
            ...
            if success:
                content_length = len(response) if isinstance(response, bytes) else 0
                self.log(f"✅ Format {i+1} works: {content_length} bytes")
            else:
                self.log(f"❌ Format {i+1} failed")
